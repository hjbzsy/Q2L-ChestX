...
def parser_args():
    ...
    # 结果输出位置
    parser.add_argument('--output', default='./out2_25', metavar='DIR',
...
def validate(val_loader, model, criterion, args, logger):
    ...
    # print("meanAUC {}".format(sum(auc_scores) / len(auc_scores)))
    # print("unhealthy {}".format((sum(auc_scores)-auc_scores[9]) / 14))
    logger.info("AUC {}".format(auc_scores))
    logger.info("meanAUC {}".format(sum(auc_scores) / len(auc_scores)))
    return loss_avg, mAP
